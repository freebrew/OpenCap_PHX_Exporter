...
- **TD calculator** (Data `I53:M55`) — Actual is last Slidesheet survey projected to bit. Planned TD is the last imported plan station. Actual TD is Planned TD plus the 3D min-curve difference from the bit vs the plan at the same MD (both inverted to Planned TD). On plan it matches Planned TD; the old 2D NS/EW chord was ~50 m short in the build.
...
- **Other Tools only on the selected BHA** — 3rd-party table is FieldCap Other Tools (`Other Inventory` / `DD other`) on `H3`. DD inventory tools, rentals, and drill bits are not seeded.
- **Actual TD is 3D min-curve vs plan** — `I55` is no longer the 2D remaining-NS/EW × azimuth RF (that ignored TVD and under-called TD by ~50 m in the build). It is Planned TD + (min-curve bit→TD − min-curve plan@MD→TD).
