...
Public Sub InitDDTools()
    ScreenBeginBusy "DD Tools: creating sheet..."
    ...
    MsgBox "DD Tools sheet created." & vbLf & _
           "Place your three OpenCap CSV exports in the OpenCap folder next to this workbook," & vbLf & _
...
Public Sub RefreshData()
    ...
    ' Freeze the screen for the WHOLE refresh.  The nested routines below use the
    ' same reference-counted guard, so none of them can thaw the screen early
    ' and let Excel repaint a half-built sheet.
    ScreenBeginBusy "Refresh: importing all OpenCap CSVs..."

    On Error GoTo ErrHandler

    ' 1) Full OpenCap import: Setup sheets, Costs sync, Setup UI
    RefreshAllCsvData
    ...
    If fJob <> "" Then ImportCSVToSheet fJob, Worksheets(SH_JOB)
    If fCrew <> "" Then ImportCSVToSheet fCrew, Worksheets(SH_CREW)
    If fBha <> "" Then ImportCSVToSheet fBha, Worksheets(SH_BHA)

    If fJob <> "" And fCrew <> "" And fBha <> "" Then
        Application.StatusBar = "Refresh: building DD Tools dashboard..."
        If SheetExists(SH_UI) Then BuildUI
    End If

    ' Data!H35:M47 mirrors DD Tools via formulas; keep that range unlocked when sheet is protected
    ...
    If fJob = "" Then missing = missing & vbLf & "  *job-details*.csv"
    If fCrew = "" Then missing = missing & vbLf & "  *crew*.csv"
...
ErrHandler:
    ScreenForceReset
    MsgBox "Error " & Err.Number & ": " & Err.Description, vbCritical, "Refresh"
End Sub
...
Public Sub RebuildDashboard()
    ...
    ScreenBeginBusy "DD Tools: rebuilding..."
...
Private Sub BuildUI()
    ...
    uiStep = "unprotect DD Tools"
    ddWasProt = SheetUnprotectForVba(ws)
    ...
    uiStep = "reprotect DD Tools"
    SheetReprotectAfterVba ws, ddWasProt
    Exit Sub
